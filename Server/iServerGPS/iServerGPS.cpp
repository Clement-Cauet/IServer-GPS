...
void iServerGPS::processTextMessage(const QString & message)
{
	...
	else if (message.section(";", 0, 0) == "serial")
	{
		...
		this->serialPort->setPortName("COM1");
	...
	else if (message.section(";", 0, 0) == "tcp")
	{
		...
		this->tcpSocket->connectToHost(QHostAddress("192.168.65.107"), this->tcpport);
...
void iServerGPS::insertValue()
{
	...
	QSqlQuery query;
	query.prepare("INSERT INTO `iServerGPS`(`latitude`, `longitude`) VALUES(:latitude, :longitude)");
